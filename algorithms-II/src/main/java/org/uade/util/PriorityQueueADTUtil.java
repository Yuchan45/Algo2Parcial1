...
public class PriorityQueueADTUtil extends BaseUtil {
    ...
    public static void print(PriorityQueueADT queue) {
        ...
        while (!copy.isEmpty()) {
            System.out.println(copy.getElement() + " (" + copy.getPriority() + ")");
            copy.remove();
        }
    }

    public static PriorityQueueADT copy(PriorityQueueADT queue) {
        PriorityQueueADT aux = getNewPriorityQueue(queue);
        ...
        PriorityQueueADT copy = getNewPriorityQueue(queue);

        while (!queue.isEmpty()) {
            int element = queue.getElement();
            int priority = queue.getPriority();
            copy.add(element, priority);
            aux.add(element, priority);
            queue.remove();
        }
    ...
    private static PriorityQueueADT getNewPriorityQueue(PriorityQueueADT queue) {
        if (queue instanceof StaticPriorityQueueADT) {
